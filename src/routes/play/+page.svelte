<script lang="ts">
  ...
  async function printCommander(deckId: string) {
    ...
    busy = true; error = ""; status = `Printing ${commander.name}…`;
    try {
      const response = await fetch("/api/game-print", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId: session.sessionId, cards: [{ id: commander.scryfallId, label: { deckId: commander.deckId, deckName: commander.deckName, cardNumber: commander.cardNumber } }] }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "Commander print failed");
      status = `Printed commander: ${commander.name}.`;
    } catch (cause) {
      error = cause instanceof Error ? cause.message : "Commander print failed";
      status = "";
    } finally { busy = false; }
  }
  ...
  function searchCard(deckId: string, card: CardInstance) { const term = (search[deckId] ?? "").trim(); if (!term) return; invoke(deckId, (s) => searchLibraryToHand(s, deckId, term, card.instanceId, random)); search = { ...search, [deckId]: "" }; }
    ...
    <div class="deck-grid">
      {#each deckEntries as [deckId, zones]}
        ...
        <section class="deck-panel" aria-labelledby={`deck-${deckId}`}>
          ...
          <div class="actions primary-actions"><button type="button" onclick={() => invoke(deckId, (s) => drawOpeningHand(s, deckId, random))} disabled={zones.library.length === 0 || session.openingHands.includes(deckId)}>Opening hand</button><button type="button" onclick={() => invoke(deckId, (s) => mulligan(s, deckId, random))} disabled={session.keptHands.includes(deckId) || !session.openingHands.includes(deckId)}>Mulligan</button><button type="button" onclick={() => invoke(deckId, (s) => keepHand(s, deckId))} disabled={!session.openingHands.includes(deckId) || session.keptHands.includes(deckId)}>Keep hand</button><button type="button" onclick={() => invoke(deckId, (s) => drawCard(s, deckId, random))} disabled={!zones.library.length}>Draw</button></div>
          ...
          <div class="land-row"><span>Basic land</span>{#each BASIC_LANDS as land}<button type="button" onclick={() => invoke(deckId, (s) => basicLandToHand(s, deckId, land, random))} disabled={!cardsIn(deckId, "library").some((card) => card.name.toLowerCase() === land.toLowerCase())}>{land}</button>{/each}</div>
