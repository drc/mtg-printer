...
<svelte:head><title>MTG Proxy Printer</title><meta name="description" content="Print Magic: The Gathering proxies" /></svelte:head>
<main>
  <h1>MTG Proxy Printer</h1>
  <section aria-labelledby="search-heading">
    <h2 id="search-heading">Find cards</h2>
    <label for="search">Search Scryfall</label>
    <input id="search" bind:value={query} oninput={scheduleSearch} placeholder="Lightning Bolt" autocomplete="off" />
    {#if results.length}<ul class="results">
        {#each results as card (card.id)}<li>
            <button onclick={() => add(card)}
              ><img src={card.image_url} alt="" />
              <span>{card.name}<small>{card.set.toUpperCase()} · {card.set_name} · #{card.collector_number}</small></span></button>
          </li>{/each}
      </ul>{/if}
  </section>
  ...
  <section aria-labelledby="link-heading">
    ...
    <input id="card-url" bind:value={cardUrl} type="url" placeholder="https://scryfall.com/card/slz/3/eerie-interlude" />
  ...
  <section aria-labelledby="queue-heading">
    ...
    {#if queue.length}<ul class="queue">
        {#each queue as card, index (index)}<li>
            <img src={card.image_url} alt={card.name} /><span>{card.name}<small>{card.set_name} · {card.collector_number}</small></span><button
